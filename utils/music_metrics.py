...
def num_notes(play,replay):
    ...
    if CUDA:
        ...
        shifted_play = torch.zeros(batch_size, time_scale, note_num)
...
def QN(sound, qualified_len = 3):
    """ratio of “qualied” notes (in %). We consider a note no shorter than three time steps
        as a qualied note. QN shows if the music is overly fragmented."""
